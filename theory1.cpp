...
int extended_point[MAXHEIGHT*(MAXHEIGHT*MAXHEIGHT+1)+1],        sizeof_extened_point ; 
...
signed main() {
    ...
    //step2: 初始化，创建 x_0 ~ x_{MAXWIDTH-1} 和 入度为0的点（矩阵的行）
    initialization() ;
    

    //step3: 指定轮数，使得结果按照贪心策略进行收敛
    ...
    //step4: 收敛轮数结束， 将图中点上没有流出的有效位 与 x_0 ~ x_{MAXWIDTH-1} 相连
